...
Проєкти редагуються в Internal → CRM: Проєкти. Кожен опублікований запис створює сторінку `/projects/{id}.html`; не змінюйте slug малими латинськими літерами, якщо не потрібно змінити URL. Публічна галерея робіт показує зображення, а сторінка проєкту має велике вибране зображення та клікабельні мініатюри під ним.

Спочатку додайте зображення-обкладинку, потім необов’язкові зображення галереї. Перетягуйте елементи галереї в Internal, щоб змінити порядок показу. Додайте alt-текст українською та англійською для обкладинки і кожного зображення галереї; підписи необов’язкові. Заповнюйте повний текст обома мовами. Редактор може вмикати або вимикати автопрокрутку галереї та встановлювати інтервал від 1 до 60 секунд (типово: 5 секунд). Сторінка проєкту містить посилання на окрему сторінку контактів `/contact.html`. Після завершення натисніть Publish і зачекайте автоматичного збирання.
...
Потрібен Node.js 20.19+ або 22.12+. `npm run dev` генерує сторінки проєктів перед запуском Vite; локально відкривайте проєкт за адресою `/projects/{id}.html`.
